    ...
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.topBar}>
          <TouchableOpacity onPress={() => router.back()} style={styles.backButton}><Ionicons name="arrow-back" size={22} color={C.text} /></TouchableOpacity>
          <Text style={styles.title}>Configurações</Text><View style={styles.topSpacer} />
        </View>

        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Meu perfil</Text>
          <View style={styles.avatarWrap}>
            {avatarUrl ? <Image source={{ uri: avatarUrl }} style={styles.avatar} /> : <View style={styles.avatarFallback}><Ionicons name="person" size={42} color={C.white} /></View>}
            <TouchableOpacity style={styles.avatarButton} onPress={pickAvatar} disabled={saving}><Ionicons name="camera" size={18} color={C.white} /><Text style={styles.avatarButtonText}>Alterar foto</Text></TouchableOpacity>
          </View>
          <Text style={styles.label}>Nome</Text>
          <TextInput value={name} onChangeText={setName} placeholder="Seu nome" placeholderTextColor={C.text3} style={styles.input} />
        ...
        <View style={styles.card}>
          ...
          <TextInput value={currentPassword} onChangeText={setCurrentPassword} secureTextEntry placeholder="Senha atual" placeholderTextColor={C.text3} style={styles.input} />
          <Text style={styles.label}>Nova senha</Text>
          <TextInput value={newPassword} onChangeText={setNewPassword} secureTextEntry placeholder="Nova senha" placeholderTextColor={C.text3} style={styles.input} />
          <Text style={styles.label}>Confirmar nova senha</Text>
          <TextInput value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry placeholder="Repita a nova senha" placeholderTextColor={C.text3} style={styles.input} />
          <TouchableOpacity style={styles.primaryButton} onPress={savePassword} disabled={passwordSaving}>{passwordSaving ? <ActivityIndicator color={C.white} /> : <Text style={styles.primaryText}>Alterar senha</Text>}</TouchableOpacity>
        </View>

        <View style={[styles.card, styles.dangerCard]}>
          <Text style={styles.sectionTitle}>Conta</Text>
          <Text style={styles.helper}>Excluir a conta remove seus dados pessoais e o acesso ao aplicativo. Suas denúncias permanecerão no sistema para preservar o histórico da cidade.</Text>
          <TouchableOpacity style={styles.deleteButton} onPress={() => setDeleteOpen(true)} disabled={deleting}><Ionicons name="trash-outline" size={19} color={C.danger} /><Text style={styles.deleteText}>Excluir minha conta</Text></TouchableOpacity>
        </View>
      </ScrollView>
