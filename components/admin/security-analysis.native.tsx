...
export default function SecurityAnalysisNative() {
  ...
  const weekly = useMemo(() => {
    ...
    reports.forEach(report => { if (!report.created_at) return; const date = new Date(report.created_at); const current = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime(); const diff = Math.floor((current - start.getTime()) / 86400000); if (diff >= 0 && diff < 7) counts[diff].count++; });
    return counts;
  }, [reports]);
  const areas = useMemo(() => { const map: Record<string, number> = {}; reports.forEach(report => { const name = report.location?.address?.split(',')[0] || 'Localização desconhecida'; map[name] = (map[name] || 0) + 1; }); return Object.entries(map).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count).slice(0, 5); }, [reports]);
  const max = Math.max(...weekly.map(item => item.count), 1); const weekTotal = weekly.reduce((sum, item) => sum + item.count, 0);
